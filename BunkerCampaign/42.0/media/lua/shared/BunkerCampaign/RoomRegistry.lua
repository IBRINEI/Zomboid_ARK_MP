...
local function normalizeRegion(region, fallbackZ)
    if type(region) ~= "table" then return nil end
    local result = {
        x1=number(region.x1, 0), x2=number(region.x2, 0),
        y1=number(region.y1, 0), y2=number(region.y2, 0),
        z=math.floor(number(region.z, fallbackZ or 0)),
    }
    if result.x2 < result.x1 then result.x1, result.x2 = result.x2, result.x1 end
    if result.y2 < result.y1 then result.y1, result.y2 = result.y2, result.y1 end
    return result
end

local function footprintArea(regions)
    local tiles = {}
    for _, region in ipairs(regions) do
        for x=math.floor(region.x1), math.floor(region.x2) do
            for y=math.floor(region.y1), math.floor(region.y2) do
                tiles[tostring(x) .. ":" .. tostring(y) .. ":" .. tostring(region.z)] = true
            end
        end
    end
    local count = 0
...
local function normalizeDefinition(definition)
    ...
    local rawBounds = type(definition.bounds) == "table" and definition.bounds or nil
    local regions = {}
    for _, region in ipairs(type(definition.regions) == "table" and definition.regions or {}) do
        local normalizedRegion = normalizeRegion(region, rawBounds and rawBounds.z or 0)
        if normalizedRegion then regions[#regions + 1] = normalizedRegion end
    end
    if not rawBounds and #regions == 0 then return nil, "room_bounds_required" end
    local bounds = rawBounds and normalizeRegion(rawBounds, 0) or {
        x1=regions[1].x1, x2=regions[1].x2, y1=regions[1].y1, y2=regions[1].y2, z=regions[1].z,
    }
    for _, region in ipairs(regions) do
        if region.z ~= bounds.z then return nil, "room_regions_must_share_level" end
        bounds.x1, bounds.x2 = math.min(bounds.x1, region.x1), math.max(bounds.x2, region.x2)
        bounds.y1, bounds.y2 = math.min(bounds.y1, region.y1), math.max(bounds.y2, region.y2)
    end
    if #regions == 0 then regions[1] = normalizeRegion(bounds, bounds.z) end
    local normalized = {
        id = definition.id,
        ...
        bounds = bounds,
        regions = regions,
        footprintArea = footprintArea(regions),
    ...
    if normalized.volumeM3 <= 0 then
        normalized.volumeM3 = normalized.footprintArea * normalized.heightMeters
    end
...
function RoomRegistry.contains(definition, x, y, z)
    ...
    local px, py, pz = number(x, -1000000000), number(y, -1000000000), math.floor(number(z, 0))
    for _, region in ipairs(type(definition.regions) == "table" and definition.regions or {bounds}) do
        if pz == region.z and px >= region.x1 and px <= region.x2 + 0.9999
            and py >= region.y1 and py <= region.y2 + 0.9999 then return true end
    end
    return false
end
...
function RoomRegistry.find(x, y, z)
    ...
    for _, definition in ipairs(RoomRegistry.all()) do
        if RoomRegistry.contains(definition, x, y, z) then
            local area = number(definition.footprintArea, math.huge, 1)
