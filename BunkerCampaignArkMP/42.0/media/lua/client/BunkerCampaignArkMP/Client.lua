...
local function addContextOptions(playerNum, context, worldObjects, test)
    ...
    if isAdmin() or getAccessLevel() == "admin" then
        ...
        context:addOption("Bunker Campaign: Add test toxic zone", player, function(p)
            sendClientCommand(p, "BunkerCampaignToxicMP", "addZone", {
                name="MPForkTest", startX=9960, startY=12615, endX=9972, endY=12630,
            })
        end)
        context:addOption("Bunker Campaign: Remove test toxic zone", player, function(p)
            sendClientCommand(p, "BunkerCampaignToxicMP", "removeZone", { name="MPForkTest" })
        end)
